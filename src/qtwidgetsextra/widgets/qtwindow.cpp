...
void QtWindowPrivate::onWindowDisplayModeChanged()
{
    ...
    if(windowDisplayMode == QtApplicationType::WindowDisplayMode::Normal &&
        windowPaintMode == QtWindowType::Movie)
    {
        windowPaintMovie->setFileName(
            themeMode == QtThemeType::Light ? lightWindowMoviePath : darkWindowMoviePath);
        windowPaintMovie->start();
    ...
    , d_ptr(new QtWindowPrivate(this))
{
    ...
    d->windowPaintMode = QtWindowType::PaintMode::Normal;
    d->lightWindowPix = new QPixmap();
    d->darkWindowPix = new QPixmap();

    d->windowPaintMovie = new QMovie(this);
...
void QtWindow::setUserInfoCardVisible(bool isVisible)
{
    Q_D(QtWindow);
    d->navigationBar->setUserInfoCardVisible(isVisible);
}
void QtWindow::setUserInfoCardPixmap(QPixmap pix)
{
    Q_D(QtWindow);
    d->navigationBar->setUserInfoCardPixmap(pix);
}
void QtWindow::setUserInfoCardTitle(QString title)
{
    Q_D(QtWindow);
    d->navigationBar->setUserInfoCardTitle(title);
}
void QtWindow::setUserInfoCardSubTitle(QString subTitle)
{
    Q_D(QtWindow);
    d->navigationBar->setUserInfoCardSubTitle(subTitle);
}

QtNavigationType::NodeResult QtWindow::addExpanderNode(
    const QString& expanderTitle, QString& expanderKey, QtIconType::IconName awesome) const
{
    Q_D(const QtWindow);
    return d->navigationBar->addExpanderNode(expanderTitle, expanderKey, awesome);
}
QtNavigationType::NodeResult QtWindow::addExpanderNode(const QString& expanderTitle,
    QString& expanderKey, const QString& targetExpanderKey, QtIconType::IconName awesome) const
{
    Q_D(const QtWindow);
    return d->navigationBar->addExpanderNode(
        expanderTitle, expanderKey, targetExpanderKey, awesome);
}
QtNavigationType::NodeResult QtWindow::addPageNode(
    const QString& pageTitle, QWidget* page, QtIconType::IconName awesome)
{
    Q_D(QtWindow);
    auto returnType = d->navigationBar->addPageNode(pageTitle, page, awesome);
    if(returnType == QtNavigationType::Success)
    {
        d->pageMetaMap.insert(page->property("QtPageKey").toString(), page->metaObject());
    }
    return returnType;
}
...
{
    Q_D(QtWindow);
    auto returnType = d->navigationBar->addPageNode(pageTitle, page, keyPoints, awesome);
    if(returnType == QtNavigationType::Success)
    {
        d->pageMetaMap.insert(page->property("QtPageKey").toString(), page->metaObject());
    }
...
QtNavigationType::NodeResult QtWindow::addPageNode(const QString& pageTitle, QWidget* page,
    const QString& targetExpanderKey, QtIconType::IconName awesome)
{
    Q_D(QtWindow);
    auto returnType = d->navigationBar->addPageNode(pageTitle, page, targetExpanderKey, awesome);
    if(returnType == QtNavigationType::Success)
    {
        d->pageMetaMap.insert(page->property("QtPageKey").toString(), page->metaObject());
    }
    ...
    const QString& targetExpanderKey, int keyPoints, QtIconType::IconName awesome)
{
    Q_D(QtWindow);
    auto returnType =
        d->navigationBar->addPageNode(pageTitle, page, targetExpanderKey, keyPoints, awesome);
    if(returnType == QtNavigationType::Success)
    {
        d->pageMetaMap.insert(page->property("QtPageKey").toString(), page->metaObject());
    }
...
QtNavigationType::NodeResult QtWindow::addFooterNode(const QString& footerTitle, QString& footerKey,
    int keyPoints, QtIconType::IconName awesome) const
{
    Q_D(const QtWindow);
    return d->navigationBar->addFooterNode(footerTitle, nullptr, footerKey, keyPoints, awesome);
}
QtNavigationType::NodeResult QtWindow::addFooterNode(const QString& footerTitle, QWidget* page,
    QString& footerKey, int keyPoints, QtIconType::IconName awesome)
{
    Q_D(QtWindow);
    auto returnType =
        d->navigationBar->addFooterNode(footerTitle, page, footerKey, keyPoints, awesome);
    if(page && returnType == QtNavigationType::Success)
    {
        d->pageMetaMap.insert(page->property("QtPageKey").toString(), page->metaObject());
    }
    return returnType;
}
QtNavigationType::NodeResult QtWindow::addCategoryNode(
    const QString& categoryTitle, QString& categoryKey)
{
    Q_D(QtWindow);
    return d->navigationBar->addCategoryNode(categoryTitle, categoryKey);
}
QtNavigationType::NodeResult QtWindow::addCategoryNode(
    const QString& categoryTitle, QString& categoryKey, const QString& targetExpanderKey)
{
    Q_D(QtWindow);
    return d->navigationBar->addCategoryNode(categoryTitle, categoryKey, targetExpanderKey);
}

// bool QtWindow::navigationNodeIsExpanded(QString expanderKey) const
// {
//     Q_D(const QtWindow);
// }
void QtWindow::expandNavigationNode(QString expanderKey)
{
    Q_D(QtWindow);
    d->navigationBar->expandNode(expanderKey);
}
// void QtWindow::collapseNavigationNode(QString expanderKey)
// {
//     Q_D(QtWindow);
// }
// void QtWindow::removeNavigationNode(QString nodeKey) const
// {
//     Q_D(const QtWindow);
// }
// int QtWindow::pageOpenInNewWindowCount(QString nodeKey) const
// {
//     Q_D(const QtWindow);
// }
void QtWindow::backtrackNavigationNode(QString nodeKey)
{
    Q_D(QtWindow);
    const QMetaObject* meta = d->pageMetaMap.value(nodeKey);
    if(!meta)
    {
        return;
    }
    ...
    if(widget)
    {
        auto originWidget = d->routeMap[nodeKey];
        int currentIndex =
            d->navigationCenterStackedWidget->containerStackedWidget()->currentIndex();
        int originIndex =
            d->navigationCenterStackedWidget->containerStackedWidget()->indexOf(originWidget);
        widget->setProperty("QtPageKey", nodeKey);
        d->routeMap[nodeKey] = widget;
        d->navigationCenterStackedWidget->containerStackedWidget()->insertWidget(
            originIndex, widget);
        d->navigationCenterStackedWidget->containerStackedWidget()->setCurrentIndex(currentIndex);
        d->navigationCenterStackedWidget->containerStackedWidget()->removeWidget(originWidget);
        originWidget->deleteLater();
    }
}

// void QtWindow::setNodeKeyPoints(QString nodeKey, int keyPoints)
// {
//     Q_D(QtWindow);
// }
// int QtWindow::nodeKeyPoints(QString nodeKey) const
// {
//     Q_D(const QtWindow);
// }

// void QtWindow::setNavigationNodeTitle(QString nodeKey, QString nodeTitle)
// {
//     Q_D(QtWindow);
// }
// QString QtWindow::navigationNodeTitle(QString nodeKey) const
// {
//     Q_D(const QtWindow);
// }

// void QtWindow::navigation(QString pageKey)
// {
//     Q_D(QtWindow);
// }
// int QtWindow::currentNavigationIndex() const
// {
//     Q_D(const QtWindow);
// }
// QString QtWindow::currentNavigationPageKey() const
// {
//     Q_D(const QtWindow);
// }

// QList<QtSuggestBox::SuggestData> QtWindow::navigationSuggestDataList() const
// {
//     Q_D(const QtWindow);
// }

// void QtWindow::setWindowButtonFlag(QtAppBarType::ButtonType buttonFlag, bool isEnable)
// {
//     Q_D(QtWindow);
// }
// void QtWindow::setWindowButtonFlags(QtAppBarType::ButtonFlags buttonFlags)
// {
//     Q_D(QtWindow);
// }
// QtAppBarType::ButtonFlags QtWindow::windowButtonFlags() const
// {
//     Q_D(const QtWindow);
// }

// void QtWindow::setWindowMoviePath(QtThemeType::ThemeMode themeMode, const QString& moviePath)
// {
//     Q_D(const QtWindow);
// }
// QString QtWindow::windowMoviePath(QtThemeType::ThemeMode themeMode) const
// {
//     Q_D(const QtWindow);
// }

// void QtWindow::setWindowPixmap(QtThemeType::ThemeMode themeMode, const QPixmap& pixmap)
// {
//     Q_D(const QtWindow);
// }
// QPixmap QtWindow::windowPixmap(QtThemeType::ThemeMode themeMode) const
// {
//     Q_D(const QtWindow);
// }

// void QtWindow::setWindowMovieRate(qreal rate)
// {
//     Q_D(QtWindow);
// }
// qreal QtWindow::windowMovieRate() const
// {
//     Q_D(const QtWindow);
// }
